...
void gpio_set_mode(struct gpio *gpio, uint8_t pin_number, uint8_t mode) {
    ...
    gpio->MODER &= ~(3U << (pin_number * 2)); // Clear existing setting
    gpio->MODER |= (mode & 3) << (pin_number * 2); // Set new mode
}
...
void gpio_set_af(struct gpio *gpio, uint8_t pin_number, uint8_t af_num) {
    gpio->AFR[pin_number >> 3] &= ~(15UL << ((pin_number & 7) * 4));
    gpio->AFR[pin_number >> 3] |= ((uint32_t) af_num) << ((pin_number & 7) * 4);
}
